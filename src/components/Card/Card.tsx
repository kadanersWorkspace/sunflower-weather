...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-slot="card"
      className={cn(
        "relative text-card-foreground flex flex-col rounded-xl shadow-sm gap-1.5 bg-transparent border-none",
        className
      )}
      {...props}
    />
...
function CardBackground({ className, ...props }: React.ComponentProps<"img">) {
  return (
    <div
      ...
      className={cn(
        "absolute rounded-xl overflow-hidden inset-0 w-full h-full -z-1",
        className
      )}
    >
